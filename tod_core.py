...
from tod_gaussian import _gaussian_accum_flatsky, _gaussian_interp_accum
...
def beam_tod_batch(
    ...
    * **Full double-Rodrigues** *(fallback)* — used when no cache arrays are
    ...
    if interp_mode not in ("nearest", "bilinear", "gaussian"):
        raise ValueError(
            f"interp_mode {interp_mode!r} not available on this branch; "
            "use the 'bicubic' branch for bicubic interpolation"
        )
