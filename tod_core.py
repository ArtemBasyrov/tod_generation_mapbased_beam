...
def _gather_accum_bicubic_jit(vec_rot, B, Sc, nside, h_pix, mp_stacked, beam_vals,
    ...
    for b in numba.prange(B):
        for s in range(Sc):
            ...
            w_sum = 0.0
            acc   = np.zeros(C)   # Numba stack-allocates this (C = 3 for T/Q/U)
            ...
            for ir in range(ir_lo, ir_hi + 1):
                ...
                if n_p <= 4:
                    # ir = 1 (only 4 pixels): include all directly to avoid
                    # duplicate indices from modulo wrapping.
                    for ip in range(n_p):
                        phi_n  = phi0 + ip * dphi_ring
                        p      = fp + ip
                        dphi_c = phi_n - ph
