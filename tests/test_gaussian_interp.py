...
class TestGaussianInterpAccum:
    ...
    def test_constant_map_gives_constant(self):
        ...
        B, Sc = 4, 8
        val = 3.7
        theta_flat, phi_flat, mp_stacked, beam_vals, tod_arr = self._build_inputs(
...
class TestBeamTodBatchGaussianOriginalPath:
    ...
    def test_gaussian_vs_bilinear_constant_map(self):
        ...
        tod_ga = beam_tod_batch(
            nside, mp, data, rot_vecs, phi_b, theta_b, psis_b, interp_mode="gaussian"
        )
        for comp in data["comp_indices"]:
    ...
    def test_sigma_affects_result(self):
        ...
        for comp in data["comp_indices"]:
            if not np.allclose(tod_narrow[comp], tod_wide[comp], atol=1e-4):
                ...
                break
        assert any_diff, (
            "Different sigma_deg values should produce different TOD on a non-flat map"
        )
...
class TestBeamTodBatchGaussianCachedPaths:
    ...
    def _build_data_with_cache(S=30, N_psi=8, nside=32, include_flatsky=False):
        ...
        vec_rolled = np.tile(base["vec_orig"][np.newaxis], (N_psi, 1, 1)).astype(
            np.float32
        )

        mp = _constant_maps(nside)
        mp_stacked = np.stack([mp[c] for c in base["comp_indices"]]).astype(np.float32)
    ...
    def test_flatsky_output_shape_dtype(self):
        ...
        nside = 32
        B = 5
        data = self._build_data_with_cache(
            S=20, N_psi=8, nside=nside, include_flatsky=True
        )
        phi_b, theta_b, psis_b, rot_vecs = _build_scan(B)
        mp = _constant_maps(nside)
...
class TestGaussianInterpAccumJit:
    ...
    @staticmethod
    def _ref_accum(
        theta_flat, phi_flat, B, Sc, nside, mp_stacked, beam_vals, sigma_rad, radius_rad
    ):
        ...
        for b in range(B):
            for s in range(Sc):
                ...
                pix_idx = hp.query_disc(
                    nside, vec_q, radius_rad, inclusive=True, nest=False
                )
                if pix_idx.size == 0:
                ...
                cos_d = np.sin(th) * np.sin(th_n) * np.cos(ph - ph_n) + np.cos(
                    th
                ) * np.cos(th_n)
    ...
    def test_accumulates_inplace(self):
        ...
        self._call_jit(
            theta_flat, phi_flat, B, Sc, nside, mp_stacked, beam_vals, tod_arr, **kwargs
        )
        first = tod_arr.copy()
        self._call_jit(
            theta_flat, phi_flat, B, Sc, nside, mp_stacked, beam_vals, tod_arr, **kwargs
    ...
    def test_parallel_matches_serial(self):
        ...
        np.testing.assert_allclose(
            tod_par, tod_ser, atol=1e-12, err_msg="Parallel and serial results differ"
        )
