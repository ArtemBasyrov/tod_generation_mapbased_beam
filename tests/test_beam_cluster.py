...
def _make_two_lobe_beam(S=300, spread=0.02, rng=None):
    ...
    vec = np.concatenate([_v(theta_A, phi_A), _v(theta_B, phi_B)], axis=0).astype(np.float32)
...
class TestKmeansPlusPlusInit:
    ...
    def test_returns_K_centroids(self):
        ...
        K = 7
        centroids = _kmeans_plus_plus_init(vec.astype(np.float64),
                                           bv.astype(np.float64), K, rng)
    ...
    def test_deterministic_with_same_seed(self):
        ...
        K = 4
        c1 = _kmeans_plus_plus_init(vec.astype(np.float64), bv.astype(np.float64),
                                     K, np.random.default_rng(99))
        c2 = _kmeans_plus_plus_init(vec.astype(np.float64), bv.astype(np.float64),
                                     K, np.random.default_rng(99))
...
class TestKmeansSphere:
    ...
    def test_output_shapes(self):
        ...
        centroids, labels = _kmeans_sphere(
            vec.astype(np.float64), bv.astype(np.float64),
            K, max_iter=20, tol=1e-4, rng=rng, verbose=False)
        assert centroids.shape == (K, 3)
    ...
    def test_centroids_on_unit_sphere(self):
        ...
        centroids, _ = _kmeans_sphere(
            vec.astype(np.float64), bv.astype(np.float64),
            8, max_iter=30, tol=1e-5, rng=rng, verbose=False)
        norms = np.linalg.norm(centroids, axis=1)
    ...
    def test_all_clusters_populated(self):
        ...
        _, labels = _kmeans_sphere(
            vec.astype(np.float64), bv.astype(np.float64),
            K, max_iter=50, tol=1e-5, rng=rng, verbose=False)
    ...
    def test_two_separated_lobes_assigned_to_different_clusters(self):
        ...
        _, labels = _kmeans_sphere(
            vec.astype(np.float64), bv.astype(np.float64),
            2, max_iter=100, tol=1e-6, rng=rng, verbose=False)
    ...
    def test_verbose_false_suppresses_output(self, capsys):
        ...
        vec, bv = _make_beam(S=50)
        _kmeans_sphere(vec.astype(np.float64), bv.astype(np.float64),
                       5, max_iter=10, tol=1e-4, rng=rng, verbose=False)
    ...
    def test_verbose_true_prints_something(self, capsys):
        ...
        vec, bv = _make_beam(S=50)
        _kmeans_sphere(vec.astype(np.float64), bv.astype(np.float64),
                       5, max_iter=10, tol=1e-4, rng=rng, verbose=True)
...
class TestSpreadStats:
    ...
    def test_verbose_false_produces_no_output(self, capsys):
        ...
        _, labels = _kmeans_sphere(
            vec.astype(np.float64), bv.astype(np.float64),
            5, max_iter=10, tol=1e-4, rng=np.random.default_rng(0), verbose=False)
        centroids, _ = _kmeans_sphere(
            vec.astype(np.float64), bv.astype(np.float64),
            5, max_iter=10, tol=1e-4, rng=np.random.default_rng(0), verbose=False)
...
class TestClusterBeamPixelsFullMode:
    ...
    def test_deterministic_with_same_seed(self):
        ...
        vec, bv = _make_beam(S=100)
        vc1, bvc1, lbl1 = cluster_beam_pixels(vec, bv, n_clusters=10,
                                               random_state=0, verbose=False)
        vc2, bvc2, lbl2 = cluster_beam_pixels(vec, bv, n_clusters=10,
                                               random_state=0, verbose=False)
    ...
    def test_two_lobes_separate_correctly(self):
        ...
        vec, bv = _make_two_lobe_beam(S=200)
        vc, bvc, labels = cluster_beam_pixels(vec, bv, n_clusters=2,
                                              random_state=0, verbose=False)
...
class TestClusterBeamPixelsHybridMode:
    ...
    def test_output_n_main_plus_K_tail(self):
        ...
        tf = 0.05
        K  = 10
        vc, bvc, labels = cluster_beam_pixels(vec, bv, n_clusters=K,
                                              tail_fraction=tf, verbose=False)
        # n_main is the number of pixels NOT in the tail.
        sort_idx = np.argsort(bv)
        cumsum   = np.cumsum(bv[sort_idx])
        n_tail   = int(np.searchsorted(cumsum, tf, side='left')) + 1
    ...
    def test_main_pixels_preserved_exactly(self):
        ...
        vec, bv = _make_beam(S=200)
        tf = 0.05
        vc, bvc, labels = cluster_beam_pixels(vec, bv, n_clusters=10,
                                              tail_fraction=tf, verbose=False)
    ...
    def test_weights_sum_to_one(self):
        ...
        vec, bv = _make_beam(S=150)
        _, bvc, _ = cluster_beam_pixels(vec, bv, n_clusters=20,
                                        tail_fraction=0.05, verbose=False)
        assert abs(float(bvc.sum()) - 1.0) < 1e-5
    ...
    def test_every_original_pixel_has_a_label(self):
        ...
        _, _, labels = cluster_beam_pixels(vec, bv, n_clusters=15,
                                           tail_fraction=0.05, verbose=False)
    ...
    def test_large_K_tail_kept_as_is(self):
        ...
        vc, bvc, labels = cluster_beam_pixels(vec, bv, n_clusters=1000,
                                              tail_fraction=tf, verbose=False)
    ...
    def test_output_dtype_float32(self):
        ...
        vec, bv = _make_beam(S=120)
        vc, bvc, _ = cluster_beam_pixels(vec, bv, n_clusters=10,
                                         tail_fraction=0.05, verbose=False)
        assert vc.dtype  == np.float32
        assert bvc.dtype == np.float32
...
class TestClusterCachedArrays:
    ...
    def _run(self, S=100, K=10, N_psi=8, rng=None, keys=None):
        ...
        vc, bvc, labels = cluster_beam_pixels(vec, bv, n_clusters=K,
                                              tail_fraction=0.10, verbose=False)
        K_out = len(bvc)
    ...
    def test_main_pixels_are_pass_through_in_hybrid_mode(self):
        ...
        sort_idx = np.argsort(bv)
        cumsum   = np.cumsum(bv[sort_idx])
        n_tail   = int(np.searchsorted(cumsum, 0.05, side='left')) + 1
